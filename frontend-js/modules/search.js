...
export default class Search {
	...
	events() {
		...
		this.closeIcon.addEventListener('click', () => this.closeOvellay())
	...
	renderResultsHTML(posts) {
		if (posts.length) {
            ...
            <div class="list-group-item active"><strong>Search Results</strong> (${posts.length > 1 ? `${posts.length} items found` : '1 item found'})</div>
              ...
              <img class="avatar-tiny" src="${post.autor.avatar}"> <strong>${post.title}</strong>
              <span class="text-muted small">by ${post.autor.username} on ${postDate.getDate()}/${postDate.getMonth()}/${postDate.getFullYear()}</span>
            </a>`
				}).join('')}
          </div>`)
		} else {
